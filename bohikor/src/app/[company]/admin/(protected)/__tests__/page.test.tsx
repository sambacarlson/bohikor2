...
describe("DashboardPage", () => {
  ...
  it("falls back to zero counts when hook data is undefined rather than an empty array", () => {
    useUsers.mockReturnValue({ data: undefined });
    useInvitations.mockReturnValue({ data: undefined });
    useRequests.mockReturnValue({ data: undefined });
    useEvents.mockReturnValue({ data: undefined, refetch: jest.fn(), isRefetching: false });
  ...
  it("calls refetchEvents when the refresh button is clicked", async () => {
    const user = userEvent.setup();
    const refetchEvents = jest.fn();
    useEvents.mockReturnValue({ data: [], refetch: refetchEvents, isRefetching: false });
  ...
  it("shows a spinning refresh icon while events are refetching", () => {
    useEvents.mockReturnValue({ data: [], refetch: jest.fn(), isRefetching: true });
  ...
  it("falls back to the raw event_type as label and a default color for unmapped events", () => {
    useEvents.mockReturnValue({
      data: [{ id: "e1", event_type: "some_unmapped_event", created_at: "2026-05-20T12:00:00Z" }],
      refetch: jest.fn(),
      isRefetching: false,
    });
    ...
    expect(screen.getByText("some_unmapped_event")).toBeInTheDocument();
  });
});
